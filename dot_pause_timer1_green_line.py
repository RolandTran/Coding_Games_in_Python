...
game_reason = "" # Stores the reason for game over
...
def draw():
    ...
    if game_over:
        screen.fill("black")
        screen.draw.text(game_reason, center=(WIDTH // 2, HEIGHT // 2), fontsize=20, color="white")
...
def update_timer():
    ...
    global remaining_time, game_over, game_reason, paused
    ...
    if remaining_time <= 0:  # Check if the timer has reached 0
        ...
        game_reason = "Time's up! Game over!" # Set the correct game-over message
        print(game_reason)
...
def on_mouse_down(pos):
    ...
    global next_dot, lines, game_over, game_reason
