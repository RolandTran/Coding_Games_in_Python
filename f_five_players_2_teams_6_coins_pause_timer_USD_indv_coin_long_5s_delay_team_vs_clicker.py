...
# Scores for Team and clicker  players
score_team = 0
score_clicker = 0
game_over = False
...
def draw():
    ...
    # Display scores for team and clicker players
    screen.draw.text(f"Team score: {round(score_team, 2)} USD", color="black", topleft=(10, 10))
    screen.draw.text(f"Clicker score: {round(score_clicker, 2)} USD", color="black", topleft=(10, 30))
    ...
    if game_over:
        screen.fill("pink")
        if winner == "team":
            screen.draw.text(f"Team wins! {round(score_team, 2)} USD.", topleft=(100, 200), fontsize=60, color="black")
        elif winner == "clicker":
...
def time_up():
    ...
    if score_team > score_clicker:
        winner = "team"
    elif score_clicker > score_team:
        winner = "clicker"
    else:
...
def update():
    global score_team, score_clicker, paused, pause_start_time, total_pause_time
