...
# --- Game State ---
score, time_left, paused, game_over, space_pressed = 0, 90, False, False, False
...
# --- Game Loops ---
def update():
    global score, paused, space_pressed, game_over
...
def draw():
    ...
    if game_over:
        ...
        if score >= 100:
            msg = f"You Win! Final Score: {round(score, 2)} USD"
            color = "green"
        else:
            msg = f"Time's up! Final Score: {round(score, 2)} USD"
            color = "red"

        screen.draw.text(msg, center=(WIDTH//2, HEIGHT//2 + 60), fontsize=60, color=color)
