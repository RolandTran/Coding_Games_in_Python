...
def draw():
    ...
    # --- Game Over Screen ---
        ...
        for name, val, date, winner in top3:
            screen.draw.text(
                f"{name}  -  {val:.2f} USD  ({date}) [{winner}]",
                center=(WIDTH/2, y),
                fontsize=32,
                color="black"
            )
...
def end_game():
    global game_over, entering_initials, winner_score, winner_label
    game_over = True
...
def update():
    ...
    if not paused and not game_over: 
        ...
        if fox.colliderect(nickel):
            ...
            print(f"Fox score is {score_fox:.2f} USD")
            place_nickel()
