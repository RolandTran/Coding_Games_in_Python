...
def end_game():
    global game_over, score_saved
    game_over = True
    print_winner()

    # determine final winner score
    final_score = max(score_fox, score_clicker)

    # if initials entry never activated, save automatically
    if not score_saved:
        auto_name = "MAN"   # MAN = manual close (change if you want)
        add_to_top3(auto_name, final_score)
        score_saved = True
        print(f"Saved automatic close score: {auto_name} {final_score:.2f}")
